...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas && heroContainer) {
        ...
        window.addEventListener('resize', () => {
            ...
            // Height-only change = URL bar toggling — do nothing, let CSS --real-vh hold steady
        });
        preload();
        renderLoop();
    }
    ...
    if (!isMobileDevice) {
        ...
        // Sync targetScrollY on native scroll events (like dragging scrollbar or keyboard scroll)
        window.addEventListener('scroll', () => {
            if (!isScrolling) {
